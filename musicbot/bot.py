...
class MusicBot(discord.Client):
    ...
    async def cmd_whitelist(self, message, user_mentions, option, something):
        """
        Usage:
            {command_prefix}whitelist [ + | - | add | remove ] @UserName [@UserName2 ...]
        Add or remove users to the whitelist.
        Only whitelisted users can run bot commands when enabled.
        """
        ...
        for user in user_mentions.copy():
            if user.id == self.config.owner_id:
                print("[Commands:Whitelist] The owner is always whitelisted.")
        ...
        if option in ['+', 'add']:
            ...
            return Response(
                '%s users have been added to the whitelist' % (len(self.whitelist) - old_len),
                reply=True, delete_after=10
            )

        else:
            if self.whitelist.isdisjoint(user.id for user in user_mentions):
                return Response('none of those users are in the whitelist.', reply=True, delete_after=10)
                ...
                return Response(
                    '%s users have been removed from the whitelist' % (old_len - len(self.whitelist)),
                    reply=True, delete_after=10
                )
    ...
    async def cmd_playdl(self, player, channel, author, permissions, leftover_args, song_url):
        """
        Usage:
            {command_prefix}play song_link
            {command_prefix}play text to search for

        Adds the song to the playlist and immediately starts downloading.
        If a link is not provided, the first
        ...
        try:
            info = await self.downloader.extract_info(player.playlist.loop, song_url, download=True, process=False)
        except Exception as e:
    ...
    async def cmd_next(self, channel, player):
        ...
        if player.playlist.peek():       
            ...
            lines.append(nextline) #keep this all
    ...
    async def on_message(self, message):
        ...
        if self.config.white_list_check and message.author.id not in self.whitelist and message.author.id != self.config.owner_id and command != 'icave' and command != 'riot':
            safe_print("[User not whitelisted] {0.id}/{0.name} ({1})".format(message.author, message_content))
            return    
